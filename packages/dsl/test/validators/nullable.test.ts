...
describe("Validators (nullable)", () => {
  ...
  test("nullable", async () => {
    ...
    expect(await run(validators.nullable(validators.isString()), null)).toEqual(
      success()
    );
    ...
    expect(await run(validators.nullable(validators.isString()), 123)).toEqual(
      failure("string")
    );
  });
  ...
  test("maybe", async () => {
    ...
    expect(format(validates(maybeString))).toEqual(
      `(try do=(any (is-absent) (is-string)) catch=function() { ... })`
    );

    expect(await run(validators.maybe(validators.isString()), "hello")).toEqual(
      success()
    );
    expect(await run(validators.maybe(validators.isString()), null)).toEqual(
      success()
    );
    expect(
    ...
    expect(await run(validators.maybe(validators.isString()), 123)).toEqual(
      failure("string")
    );
  });
});
