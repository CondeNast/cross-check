...
function unwrapErrors(errors: ValidationError[]) {
  ...
  if (errors[0].message.name !== "multiple") {
    throw new Error(
      `Expected error to have name "multiple", but got "${errors[0].message.name}"`
    );
  }
  ...
  if (result.length !== 2) {
    throw new Error(
      `Expected error details to have 2 items, but got ${result.length} items`
    );
  }
  if (result[0][0].message.name !== "type") {
    throw new Error(
      `Expected error to have name "type", but got "${result[0][0].message.name}"`
    );
  }
